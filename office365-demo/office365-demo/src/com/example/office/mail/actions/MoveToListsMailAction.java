...
/**
 * Provides functional to allow user move email to another list
 */
public class MoveToListsMailAction extends ListItemAnimationAction<BoxedMailItem> {
     ...
     */
    public MoveToListsMailAction(AnimatedListViewTouchListener listener, Direction direction, float partition, Fragment fragment) {
        super(listener, direction, partition, android.R.drawable.ic_menu_sort_by_size, OfficeApplication.getContext().getResources()
                .getColor(R.color.background_move_to_lists));
    ...
    public void doAction(Object... params) throws IllegalArgumentException {
        try {
            ...
            fragment.startActivityForResult(intent, Constants.CHOOSE_LIST_REQUEST);
        } catch (Exception e) {
    ...
    public boolean onComplete(Object... params) {
        try {
            ...
            if (Constants.CHOOSE_LIST_REQUEST == (Integer) (params[0])) {
                complete();
                Intent data = (Intent) params[1];
                String mailId = data.getExtras().getString(EMAIL_ID_EXTRA);
