    ...
    public static void updateConfiguration(MailConfig newConfig) {
        try {
            ...
            if (localConfig != null && newConfig != null && newConfig.getMails() != null) {
                for (BoxedMailItem serverItem : newConfig.getMails()) {
                    BoxedMailItem localItem = localConfig.getMailById(serverItem.getId());
                    if (localItem != null) {
                        newConfig.updateMailById(serverItem.getId(), (BoxedMailItem)serverItem.setBox(localItem.getBox())/*.
